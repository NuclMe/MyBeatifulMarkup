...
function selectToggle(val) {
  ...
  const setActive = funcvalue.parentElement.classList.toggle('is-active')
  document.querySelector('body').addEventListener('click', (e) => {
    const target = e.target
    if (target !== funcvalue || target !== selectItem) {
      document.querySelector('.select').remove('is-active')
    } else {
      selectToggle()
    }
  })
